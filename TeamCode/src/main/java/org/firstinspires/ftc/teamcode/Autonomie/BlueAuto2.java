...
@Autonomous(name = "Blue Side Special", group = "main")
public class BlueAuto2 extends LinearOpMode{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        if (tip_autonomie == 1){
            motor_slider.setTargetPosition(1095);
        ...
        if (tip_autonomie == 2){
            motor_slider.setTargetPosition(1545);
            motor_slider.setPower(1);
            while(motor_slider.isBusy());
            motor_slider.setPower(0);
        ...
        path.add(drive.trajectoryBuilder(path.get(1).end())
                .lineToLinearHeading(new Pose2d(-2.5, -2, Math.toRadians(270)))
                .build());
